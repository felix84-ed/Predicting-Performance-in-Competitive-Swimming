...
# Nur die 14-Jährigen betrachten und fehlende Werte in der Spalte "percent_diff_improvement_14y" durch den Medianwert ersetzen
data_relevant <- data_relevant %>%
  filter(Alter == 14 | !is.na(percent_diff_improvement_14y))
...
plot_file_path <- paste0("/Users/fbongarz/Library/Mobile Documents/com~apple~CloudDocs/02_Studium/Bachelor-Arbeit/BachelorArbeit/Ergebnisse/14zu18/Schwimmzeiten_14_18y_Vorhersage_", gsub(" ", "_", filtered_discipline), "_Plot.png")
...
results_file_path <- paste0("/Users/fbongarz/Library/Mobile Documents/com~apple~CloudDocs/02_Studium/Bachelor-Arbeit/BachelorArbeit/Ergebnisse/14zu18/Schwimmzeiten_Vorhersage_14_18y_Ergebnisse_", gsub(" ", "_", filtered_discipline), ".csv")
...
# Vorhersage --------------------------------------------------------------
# Neue Daten für Vorhersage eingeben --------------------------------------
new_data <- data.frame(
  Frühestes_Alter = 11,  # Beispielwert
  X.Nr_Disziplinen.Jahr. = 5,  # Beispielwert
  X.Nr_Dist_Kat.Jahr. = 3,  # Beispielwert
  X.Nr_Jahre_Top100.Allg. = 8,  # Beispielwert
  X.Nr_Jahre_Top100.Disz. = 8,  # Beispielwert
  percent_diff_improvement_14y = 0.38,  # Beispielwert
  Alter = 14  # Alter
)

# Vorhersage für die neuen Daten durchführen
new_prediction <- predict(gbm_model, newdata = new_data, n.trees = best_trees)
