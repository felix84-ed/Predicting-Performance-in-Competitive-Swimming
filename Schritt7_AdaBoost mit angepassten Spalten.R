...
features <- c('Frühestes_Alter', 'X.Nr_Disziplinen.Jahr.', 'X.Nr_Dist_Kat.Jahr.',
              'X.Nr_Jahre_Top100.Allg.', 'X.Nr_Jahre_Top100.Disz.', 'percent_diff_improvement_11y')
target <- 'Zeit'
...
gbm_model <- gbm(Zeit ~ Frühestes_Alter + X.Nr_Disziplinen.Jahr. + X.Nr_Dist_Kat.Jahr. +
                   X.Nr_Jahre_Top100.Allg. + X.Nr_Jahre_Top100.Disz. + percent_diff_improvement_11y + Alter, 
                 data = train_data, 
...
output_file_path <- paste0("/Users/fbongarz/Library/Mobile Documents/com~apple~CloudDocs/02_Studium/Bachelor-Arbeit/BachelorArbeit/Ergebnisse/", gsub(" ", "_", filtered_discipline), "_Plot.png")
...
results <- data.frame(
  Metric = c("mse", "R²", "Alter", "percent_diff_improvement_11y", "Anzahl Distanz Kategorie im Jahr", 
  ...
  Value = c(
    ...
    importance["percent_diff_improvement_11y", "rel.inf"], 
...
# Define the output file path
output_file_path <- paste0("/Users/fbongarz/Library/Mobile Documents/com~apple~CloudDocs/02_Studium/Bachelor-Arbeit/BachelorArbeit/Ergebnisse/ada_Boost_Ergebnis_", gsub(" ", "_", filtered_discipline), ".csv")
