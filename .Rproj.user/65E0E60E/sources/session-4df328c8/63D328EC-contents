...
if(!require(nnet)) {
  install.packages("nnet")  # Enthält die Funktion für neuronale Netze
  library(nnet)
}
...
auc_values <- foreach(i = 1:5, .combine = c, .packages = c('nnet', 'pROC', 'dplyr')) %dopar% {
  
  # Trainings- und Testdaten für den Fold erstellen
  fold_train <- data_clean[folds[[i]], ]
  fold_test <- data_clean[-folds[[i]], ]
  
  # Neural Network Modell trainieren (Größe = Anzahl der Neuronen im hidden layer)
  nn_model <- nnet(group ~ Frühestes_Alter + improvement_rate + max_yearly_improvement +
                     years_in_top100 + max_dist_categories + max_disciplines + 
                     fina_16 + max_z_time_rate + z_time_ratio, 
                   data = fold_train, size = 5, maxit = 500, decay = 0.01, linout = FALSE)
  
  # Vorhersagen auf Wahrscheinlichkeitsbasis für den aktuellen Fold
  nn_probabilities <- predict(nn_model, newdata = fold_test, type = "raw")
  
  # ROC-Kurve und AUC für Gruppe 1 vs. Gruppe 2
  roc_curve <- roc(fold_test$group, nn_probabilities, levels = c(1, 2), direction = "<")
...
png("/Users/fbongarz/Library/Mobile Documents/com~apple~CloudDocs/02_Studium/Bachelor-Arbeit/Data/Ergebnisse/neural_network_roc_curve.png", 
...
write.csv(results, "/Users/fbongarz/Library/Mobile Documents/com~apple~CloudDocs/02_Studium/Bachelor-Arbeit/Data/Ergebnisse/neural_network_results.csv", row.names = FALSE)
